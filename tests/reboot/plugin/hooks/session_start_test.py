...
class SessionStartTest(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        # Codex passes this alongside the plugin root; the handler reads
        # nothing from it, but the Codex case sets it so that case is
        # the environment Codex really supplies.
        self.plugin_data = os.path.join(directory.name, "plugin-data")
    ...
    def test_no_op_without_env_file(self) -> None:
        """Codex runs the same `hooks.json` but has no equivalent of
        `$CLAUDE_ENV_FILE`, and wires the PATH prepend through
        `~/.codex/config.toml` instead. The handler must succeed
        silently rather than fail the session with a hook error."""
        cases: list[dict[str, str]] = [
            # Codex: it sets the plugin variables, pointing them at its
            # own plugin cache, and no env file.
            {
                "CLAUDE_PLUGIN_ROOT": self.plugin_root,
