...
def get_descriptor_set(name: str) -> FileDescriptorSet:
    ...
    Wrapper around test_helpers.get_descriptor_set to add shared namespacing."""
    return test_helpers.get_descriptor_set(f'tests.reboot.server.{name}')


class ServiceDescriptorValidatorTestCase(unittest.TestCase):
    ...
    non_task_method: FileDescriptorSet
    ...
    def test_removed_method(self):
        ...
        self.assertEqual(
            'Method `tests.reboot.server.EchoMethods.Replay` was deleted',
            errors[0],
        )
    ...
    def test_changed_method_type(self):
        ...
        self.assertEqual(len(errors), 1)
        self.assertEqual(
            'Reboot options for method `tests.reboot.server.EchoMethods'
            '.Replay` updated from...\n'
            '```\n'
            'reader {\n'
            '}\n'
    ...
    def test_changed_method_input_type_field_deleted(self):
        ...
        self.assertIn(
            ...
            'EchoMethods.Reply`), and its field number was '
            'not reserved.',
            errors[0],
        )
        self.assertIn(
            'Field `string_field` was deleted from message `SingleStringField` '
            '(changed to `EmptyMessage` in `tests.reboot.server.'
            'EchoMethods.Reply`), and its field name was '
            'not reserved.',
            errors[1],
        )
    ...
    def test_changed_method_input_type_field_deleted_partially_reserved(self):
        ...
        self.assertIn(
            ...
            'server.EchoMethods.Reply`), and its field '
            'name was not reserved.',
            errors[0],
        )
        self.assertIn(
    ...
    def test_changed_method_input_type_reserved_deleted(self):
        ...
        self.assertIn(
            ...
            '`EmptyMessageWithReserved` (changed to `EmptyMessage` in `tests.'
            'reboot.server.EchoMethods.Reply`).',
            errors[0],
        )
        self.assertIn(
            'Field name `string_field` is no longer `reserved` in message '
            '`EmptyMessageWithReserved` (changed to `EmptyMessage` in `tests.'
            'reboot.server.EchoMethods.Reply`).',
            errors[1],
        )
    ...
    def test_changed_method_output_type_single_to_repeated_field(self):
        """Test that changing the output type of a method, and switching a field
        from single to repeated, produces an error."""
        with self.assertRaises(ProtoValidationError) as e:
            validate_descriptor_sets_are_backwards_compatible(
                self.original,
    ...
    def test_enum_option_added(self):
        """Test that changing the input type of a method by adding an enum field
    ...
    def test_multiple_changes(self):
        ...
        self.assertEqual(
            'Method `tests.reboot.server.EchoMethods.'
            'Hello` was deleted',
            errors[1],
        )
    ...
    def test_compare_reserved_field_numbers(self):
        ...
        def range(
            ...
            # NOTE: when we write a range in a proto, e.g. "1 to 10", the end of
            # the range is _inclusive_. However, the `ReservedRange.end` is
            # _exclusive_ (!!). We do that conversion here.
        ...
        def test(
            ...
            errors = _compare_reserved_field_numbers(
                before, after, 'TestMessage', '(in test)'
            )
    ...
    def test_dict_diff(self):
        """Tests that dict_diff generates useful diffs for nested collections."""
