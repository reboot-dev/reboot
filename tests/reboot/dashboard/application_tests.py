...
class TestDashboardApplication(unittest.IsolatedAsyncioTestCase):
    """Checks that the dashboard application stands up on its own and
    ...
    async def test_presence_reports_a_connected_subscriber(self) -> None:
        ...
        # Until a writer constructs the `Presence` state, `List`
        # aborts rather than reporting an empty list. The dashboard's
        # `initialize` calls `Create` for exactly this reason, so its
        # readers always have an instance to read; this application has
        # no `initialize`, which is what lets this case be seen.
