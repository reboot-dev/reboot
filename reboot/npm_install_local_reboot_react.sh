...
###
# Install the `//reboot/react:react` package
# (AKA `@reboot-dev/reboot-react`) into the local `package.json` and
...
# Delete the old `node_modules` directory; if we don't, previous
# versions of the Reboot libraries we'd installed will not get
# overwritten.
rm -rf node_modules/
