...
def register_generate(parser: ArgumentParser):
    ...
    parser.subcommand('generate').add_argument(
        ...
        type=str,
        default=None,
        help="output directory for the React Native (mobile) client.",
    )
...
async def generate_direct(
    ...
    # The React Native (mobile) client is generated by reusing the
    # React client's TypeScript output, so the two necessarily share an
    # import style. `--react-extensions` makes those relative imports
    # carry explicit `.js` file extensions (e.g. `import "./foo_pb.js"`),
    # which is what strict ESM resolvers require. Metro, React Native's
    # bundler, is not one of them: given `./foo_pb.js` it looks for a
    # literal `foo_pb.js` on disk and never falls back to the `foo_pb.ts`
    # source the way a TypeScript "bundler"-mode resolver (or Vite) does,
    # so the mobile bundle fails with "Unable to resolve module".
    #
    # Dropping `--react-extensions` costs the web/React client nothing:
    # Vite resolves extensionless imports out of the box (its default
    # `resolve.extensions` already covers `.ts`/`.tsx`), and `tsc`
    # resolves them under `moduleResolution: "bundler"` or `"node"` —
    # the settings our React frontends already use. So no Vite or
    # tsconfig change is needed to consume the extensionless client.
    # Extensions are only mandatory under strict ESM (`"nodenext"`,
    # e.g. running the client directly under Node as the
    # `--nodejs-extensions` examples do), and that target never
    # involves Metro.
    if args.mobile is not None and args.react_extensions:
        terminal.fail(
    ...
    if returncode == 0 and mobile_copy_from is not None:
        # Mirror the generated React client into the mobile directory.
        # The mobile client uses the same TypeScript, so this avoids a
        # second `protoc` invocation.
        shutil.copytree(
            mobile_copy_from,
            args.mobile,
            dirs_exist_ok=True,
