...
// it.
//
// The value arrives as a `google.protobuf.Value`, which is
// schemaless, so its true shape is known only to `changelog.py` and
// the compiler cannot vouch for it. The zod schemas restore safety at
// that boundary: each entry is checked at runtime, and `z.infer`
// derives the static types from the same declaration.
import { z } from "zod";
