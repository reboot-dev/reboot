Feature: Adding transcripts to a wiki
  ...
  Scenario: Adding a transcript creates it
    Given "alice" does a `create_wiki` with `name="notes"` and `description=""` on `User` of "alice"
    And the resulting `wiki_id` is saved as "wiki_id"
    When "alice" does a `add_transcript` with `messages=[{role: "user", content: "Hi."}, {role: "assistant", content: "Hello!"}]` on `Wiki` of "<wiki_id>"
