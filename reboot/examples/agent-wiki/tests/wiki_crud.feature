Feature: Wiki, page, and transcript CRUD
  ...
  Scenario: A created wiki appears in the user's list
    When "alice" does a `create_wiki` with `name="my notes"` and `description="my personal notes"` on `User` of "alice"
  ...
  Scenario: A fresh wiki updates its markdown body
    Given "alice" does a `create_wiki` with `name="my notes"` and `description="my personal notes"` on `User` of "alice"
    And the resulting `wiki_id` is saved as "wiki_id"
    Then as "alice", `get` on the `Wiki` for "<wiki_id>" has `name="my notes"` and `description="my personal notes"` and `content=""`
    When "alice" does a `update` with `content="# Hello\n"` on `Wiki` of "<wiki_id>"
  ...
  Scenario: Pages round-trip their title and body
    ...
    When "alice" does a `update` with `title="Renamed Page"` and `content="New body."` on `Page` of "<page_id>"
  ...
  Scenario: Transcripts round-trip their messages
    ...
    When "alice" does a `update` with `messages=[{role: "user", content: "Goodbye"}]` on `Transcript` of "<transcript_id>"
