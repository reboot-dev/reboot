Feature: Swag store
  ...
  Scenario: Another user cannot read the cart
    ...
    And "test-user" does a `add_item` with `quantity=1` and `product_id="hoodie-1"` and `variant_id="hoodie-1-l"` and `name="Reboot Hoodie"` and `price_cents=4000` and `image_url=""` and `size="L"` on `Cart` of "<cart_id>"
  ...
  Scenario: Added items appear in the cart
    Given "test-user" creates a `Cart` via `create` with `owner_id="test-user"`
    And the resulting state id is saved as "cart_id"
    When "test-user" does a `add_item` with `quantity=2` and `product_id="hoodie-1"` and `variant_id="hoodie-1-l"` and `name="Reboot Hoodie"` and `price_cents=4000` and `image_url=""` and `size="L"` on `Cart` of "<cart_id>"
  ...
  Scenario: Adding a different variant adds a line
    Given "test-user" creates a `Cart` via `create` with `owner_id="test-user"`
    And the resulting state id is saved as "cart_id"
    When "test-user" does a `add_item` with `quantity=1` and `product_id="hoodie-1"` and `variant_id="hoodie-1-l"` and `name="Reboot Hoodie"` and `price_cents=4000` and `image_url=""` and `size="L"` on `Cart` of "<cart_id>"
    And "test-user" does a `add_item` with `quantity=1` and `product_id="hoodie-1"` and `variant_id="hoodie-1-s"` and `name="Reboot Hoodie"` and `price_cents=4000` and `image_url=""` and `size="S"` on `Cart` of "<cart_id>"
  ...
  Scenario: Removed items leave the cart
    ...
    And the resulting state id is saved as "cart_id"
    When "test-user" does a `add_item` with `quantity=1` and `product_id="hoodie-1"` and `variant_id="hoodie-1-l"` and `name="Reboot Hoodie"` and `price_cents=4000` and `image_url=""` and `size="L"` on `Cart` of "<cart_id>"
    And "test-user" does a `remove_item` with `product_id="hoodie-1"` on `Cart` of "<cart_id>"
    Then as "test-user", `get_cart` on the `Cart` for "<cart_id>" has `items=[]`

  Scenario: Checking out an empty cart is refused
    Given "test-user" creates a `Cart` via `create` with `owner_id="test-user"`
    And the resulting state id is saved as "cart_id"
    When "test-user" attempts a `checkout` with `coupon_code="000000"` and `shipping_address={name: "Jane Doe", email: "jane@example.com", address1: "123 Main St", address2: "", city: "Seattle", state_code: "WA", zip_code: "98101", country_code: "US"}` on `Cart` of "<cart_id>"
  ...
  Scenario: An invalid coupon refuses checkout and keeps the cart
    ...
    And the resulting state id is saved as "cart_id"
    And "test-user" does a `add_item` with `quantity=1` and `product_id="hoodie-1"` and `variant_id="hoodie-1-l"` and `name="Reboot Hoodie"` and `price_cents=4000` and `image_url=""` and `size="L"` on `Cart` of "<cart_id>"
    When "test-user" attempts a `checkout` with `coupon_code="definitely-not-a-real-code"` and `shipping_address={name: "Jane Doe", email: "jane@example.com", address1: "123 Main St", address2: "", city: "Seattle", state_code: "WA", zip_code: "98101", country_code: "US"}` on `Cart` of "<cart_id>"
  ...
  Scenario: Checkout empties the cart and creates the order
    ...
    And the resulting state id is saved as "cart_id"
    And "test-user" does a `add_item` with `quantity=2` and `product_id="hoodie-1"` and `variant_id="hoodie-1-l"` and `name="Reboot Hoodie"` and `price_cents=4000` and `image_url=""` and `size="L"` on `Cart` of "<cart_id>"
    When "test-user" does a `checkout` with `coupon_code=<coupon_code>` and `shipping_address={name: "Jane Doe", email: "jane@example.com", address1: "123 Main St", address2: "", city: "Seattle", state_code: "WA", zip_code: "98101", country_code: "US"}` on `Cart` of "<cart_id>"
  ...
  Scenario: A redeemed coupon cannot be reused
    ...
    And the resulting state id is saved as "cart_id"
    And "test-user" does a `add_item` with `quantity=1` and `product_id="hoodie-1"` and `variant_id="hoodie-1-l"` and `name="Reboot Hoodie"` and `price_cents=4000` and `image_url=""` and `size="L"` on `Cart` of "<cart_id>"
    And "test-user" does a `checkout` with `coupon_code=<coupon_code>` and `shipping_address={name: "Jane Doe", email: "jane@example.com", address1: "123 Main St", address2: "", city: "Seattle", state_code: "WA", zip_code: "98101", country_code: "US"}` on `Cart` of "<cart_id>"
    And "test-user" creates a `Cart` via `create` with `owner_id="test-user"`
    And the resulting state id is saved as "second_cart_id"
    And "test-user" does a `add_item` with `quantity=1` and `product_id="hoodie-1"` and `variant_id="hoodie-1-l"` and `name="Reboot Hoodie"` and `price_cents=4000` and `image_url=""` and `size="L"` on `Cart` of "<second_cart_id>"
    When "test-user" attempts a `checkout` with `coupon_code=<coupon_code>` and `shipping_address={name: "Jane Doe", email: "jane@example.com", address1: "123 Main St", address2: "", city: "Seattle", state_code: "WA", zip_code: "98101", country_code: "US"}` on `Cart` of "<second_cart_id>"
    Then the attempt aborts with `InvalidCoupon`
