Feature: Food orders
  ...
  Scenario: Starting an order pre-populates the menu with an empty cart
    When as "alice" the `User` for "alice" gets a `start_order`
  ...
  Scenario: Adding the same item twice increments its quantity
    Given as "alice" the `User` for "alice" gets a `start_order`
    And the resulting `order_id` is saved as `order_id`
    When as "alice" the `FoodOrder` for "<order_id>" gets a `add_to_cart` with `item_index=0` and `quantity=1`
    And as "alice" the `FoodOrder` for "<order_id>" gets a `add_to_cart` with `item_index=0` and `quantity=1`
    And as "alice" the `FoodOrder` for "<order_id>" gets a `add_to_cart` with `item_index=1` and `quantity=1`
    ...
    When as "alice" the `FoodOrder` for "<order_id>" gets a `remove_from_cart` with `item_index=0`
  ...
  Scenario: A quantity of zero means one
    Given as "alice" the `User` for "alice" gets a `start_order`
    And the resulting `order_id` is saved as `order_id`
    When as "alice" the `FoodOrder` for "<order_id>" gets a `add_to_cart` with `item_index=0` and `quantity=0`
  ...
  Scenario: Out-of-range menu indexes are refused
    Given as "alice" the `User` for "alice" gets a `start_order`
    And the resulting `order_id` is saved as `order_id`
    When as "alice" the `FoodOrder` for "<order_id>" attempts a `add_to_cart` with `item_index=10` and `quantity=1`
    Then the attempt aborts with `Unknown`
    When as "alice" the `FoodOrder` for "<order_id>" attempts a `add_to_cart` with `item_index=-1` and `quantity=1`
    Then the attempt aborts with `Unknown`
  ...
  Scenario: Another user cannot touch the order
    ...
    When as "bob" the `FoodOrder` for "<order_id>" attempts a `add_to_cart` with `item_index=0` and `quantity=1`
