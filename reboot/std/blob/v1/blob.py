...
from rbt.std.blob.v1.blob_rbt import (
    ...
    PerformCommitRequest,
    PerformCommitResponse,
...
class BlobServicer(Blob.Servicer):
    ...
    def authorizer(self) -> Blob.Authorizer:
        ...
        return Blob.Authorizer(
            ...
            perform_commit=allow_if(any=[is_app_internal]),
    ...
    async def begin_upload(
        ...
        # session instead. Deletion always wins, as in `PerformCommit`.
        removed = False
    ...
    async def commit(
        ...
        if self.state.status == Blob.State.COMMITTING:
            # Idempotent: the `PerformCommit` workflow is already
            # scheduled.
        ...
        await self.ref().schedule().perform_commit(context)
        return CommitResponse()
...
class BlobLibrary(Library):
    ...
    def _hosts_data_plane(self) -> bool:
        """Whether this application serves its own blob bytes. It does
        unless `REBOOT_BLOB_DATA_PLANE_URL` points it at a data plane
        elsewhere."""
        return configured_data_plane_url() is None
    ...
    async def pre_run(self, application: Application) -> None:
        ...
        configuration = await self._configuration(url)
    ...
    async def _configuration(self, url: str) -> ConfigurationResponse:
        """Asks the data plane at `url` how it is configured. The data
        plane is normally already running, but a startup race is
        tolerated by retrying while it becomes reachable."""
        backoff = Backoff(
        ...
        while True:
            try:
                async with data_plane_stub_at(url) as data_plane:
