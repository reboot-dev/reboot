...
_FORWARDED_GET_HEADERS = (
    ...
    "ETag",
    "Accept-Ranges",
    # Without this the data plane's own `nosniff` would be dropped on
    # the way through, which is exactly where it matters: these bytes
    # reach the browser on the application's origin.
...
def mount_proxy_routes(
    ...
    async def get_forward(rest: str, request: Request) -> Response:
        ...
        # Applied again on the way out, not just where the bytes
        # are stored: a data plane that is not ours -- S3, say --
        # returns whatever content type it was given at upload,
        # and this hop is what puts it on the application's
        # origin.
        media_type, safety_headers = download_headers(
