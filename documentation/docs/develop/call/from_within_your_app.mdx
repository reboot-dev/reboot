...
[servicers](/develop/implement/servicers). Each of those
...
- `ReaderContext` - Passed to a [`reader`](/develop/define/methods#kinds).
- `WriterContext` - Passed to a [`writer`](/develop/define/methods#kinds).
- `TransactionContext` - Passed to a [`transaction`](/develop/define/methods#kinds).
- `WorkflowContext` - Passed to a [`workflow`](/develop/implement/workflows).

The types of these contexts allow Reboot (as well as static type
checkers like `mypy`, `Pyright`, or `tsc`) to enforce its [safety
guarantees](/develop/define/methods) throughout the call
graph.
...
[`transaction`](/develop/define/methods#kinds) method instead.
...
See [Tasks](/develop/tasks) for more details.
