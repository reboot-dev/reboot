...
VS Code, Goose — as one more frontend alongside your
[web](/frontends/web) and [React Native](/frontends/react_native) apps.
...
[`User`](/users/overview). Nothing about a chat frontend is a separate
kind of application.
...
**[`UI` methods](/frontends/ui_methods) become tools that open a React
component.** When the AI calls one, the chat client displays your
component inline.

## Users on the chat frontend
...
This is what makes `User` the natural entry point on this frontend in
...
- Give `User` a tool that lists it, so the AI can look it up again in
...
- [`UI` methods](/frontends/ui_methods) — React components rendered in
  the conversation.
