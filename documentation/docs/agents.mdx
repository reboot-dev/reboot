...
An AI agent can reach your application from outside, over
[MCP](/frontends/ai_chat). It can also run *inside* it, as part of your
own business logic — that is what this page is about.

Reboot's `reboot.agents` package lets you run [Pydantic AI](https://ai.pydantic.dev/)
agents inside Reboot [workflows](/implement/workflows) with
...
- **Every tool call** -- whether registered via `@agent.tool`, passed
  ...
  validation](/side_effects) tools DO re-run, which helps
...
validation](/side_effects), Reboot re-runs your tool to
