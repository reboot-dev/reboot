...
[`transaction`](/define/methods#kinds), and then
call that method from your client.

## Making calls idempotently

You can make calls [`idempotently()`](/idempotency)
...
[Making concurrent calls](/call/from_within_your_app#making-concurrent-calls).
For Python, the `concurrently` helper provides adaptive
concurrency limits and streaming results — see
[`concurrently` (Python)](/call/from_within_your_app#concurrently-python)
