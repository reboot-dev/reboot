...
[React](/call/from_react) client code (or libraries that
...
[constructed](/call/overview#constructing-instances)!**
...
[`initialize`](/implement/application#initialize-functions) function
that you pass to [`Application`](/implement/application).
...
or _explicitly_, depending on whether or not you have designated a
method as a _factory_ in your Pydantic or Zod schema.

:::tip
Prefer an explicit constructor for the types your application creates
many of: it lets Reboot
[generate the state ID](/concepts#let-reboot-generate-the-id) and
gives you one place to initialize state. Implicit construction suits
singletons and other instances whose ID you choose yourself.
:::
...
Here's an example, in both Pydantic and Zod, that designates
...
<!-- MARKDOWN-AUTO-DOCS:START (CODE:src=../../../reboot/examples/bank-pydantic/api/bank/v1/account.py&syntax=python&lines=29-38) -->
...
[`idempotently()`](/idempotency) external to a Reboot
