...
[servicers](/implement/servicers). Each of those
...
- `ReaderContext` - Passed to a [`reader`](/define/methods#kinds).
- `WriterContext` - Passed to a [`writer`](/define/methods#kinds).
- `TransactionContext` - Passed to a [`transaction`](/define/methods#kinds).
- `WorkflowContext` - Passed to a [`workflow`](/implement/workflows).

The types of these contexts allow Reboot (as well as static type
checkers like `mypy`, `Pyright`, or `tsc`) to enforce its [safety
guarantees](/define/methods) throughout the call
graph.
...
via `write()` — see
[Workflows](/implement/workflows#reading-state).
...
[`transaction`](/define/methods#kinds) method instead.
