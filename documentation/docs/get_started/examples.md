...
Full-stack Reboot application examples with React frontends.
Looking for AI Chat App examples? See
...
* how to run a long-lived control loop using a [`workflow`](/define/methods#kinds) method, which takes checkpoints of the ProseMirror document
...
* different [data types](/define/overview) on the backend
...
It includes:
* a reactive frontend
* multiple instances of the same [data type](/define/overview)
* a simple [transaction](/define/methods#kinds) that
...
the simplest possible chat app: a single user, and a [singleton](/call/overview#state-ids)
"chat room". It contains:
* a reactive frontend: open two browsers pointed at the same backend and see messages flow!
* [optimistic updates](/call/from_react#optimistic-updates): sent messages are
...
Because of `hello`'s simplicity, it does not use any [transactions](/define/methods#kinds) --
...
* many different [data types](/define/overview) and API interactions
