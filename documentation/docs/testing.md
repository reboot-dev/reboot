...
Some servicers may use [secrets](/secrets) for connecting
...
[task](/tasks) half-run, a
[workflow](/implement/workflows) between steps, an effect
...
* While the app is down, don't `await` a unary call from the test's
  context. An
  [`ExternalContext`](/call/from_outside_your_app) is the
...
an [`ExternalContext`](/call/from_outside_your_app) or directly
with an [`HTTP`](/call/via_http) request.
