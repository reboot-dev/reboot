...
Every method must explicitly declare its MCP exposure with
`mcp=Tool()` (to expose it as an AI-callable tool) or
`mcp=None` (to hide it from the AI). This applies to all
state types, including `User`. See
