...
Reboot [methods](/learn_more/define/methods) can be composed
according to a safety hierarchy, which enables simple, safe, and
powerful APIs.

In order to enable that composition, Reboot methods must:
1. Isolate any side-effects that they need to execute to [tasks](/learn_more/tasks).
...
Reboot [methods](/learn_more/define/methods) twice [^1] when running in development
...
* the `--effect-validation={enabled,quiet,disabled}` flag for `rbt dev run`
