...
[`UI` method](/frontends/ui_methods) and methods marked `mcp=Tool()`
so an [MCP client](/frontends/mcp_ui) can call them:

<!-- MARKDOWN-AUTO-DOCS:START (CODE:src=../../../reboot/examples/mcp-ui-counter/api/mcp_ui_counter/v1/counter.py&syntax=python) -->
...
`Transaction`, `Workflow`, and [`UI`](/frontends/ui_methods) methods.
...
[Exclusive or shared](/implement/transactions#exclusive-or-shared).
