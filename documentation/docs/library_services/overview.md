...
Reboot's integrations add clean, surefire APIs to interact with third party services. Because interactions with external services are [isolated as side-effects](/side_effects), you can confidently use an integration's API -- knowing that it will safely compose, and without worrying about whether it might partially fail or retry unsafely.
