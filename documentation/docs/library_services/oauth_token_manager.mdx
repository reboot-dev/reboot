...
**replacing** any previously stored ones. If the new `tokens` leaves
`refresh_token` unset but one was stored before, the existing refresh
token is carried forward (some services issue a refresh token only on
the first consent).
