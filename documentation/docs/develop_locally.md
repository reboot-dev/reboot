...
| `dev run --frontend-host=<url>` | Specifies the frontend proxy host. Set to the Vite dev server URL (e.g., `http://localhost:4444`) for HMR during development, or `""` for production builds served from `web/dist/`. |
