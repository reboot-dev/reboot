...
[AI chat client](/surfaces/ai_chat) opens inside the conversation when
...
:::info The AI chat surface only
`UI` methods exist for [MCP](/surfaces/ai_chat), where they appear as
tools. There is no way to invoke one from a browser or from your
backend. Your [web](/surfaces/web) and
[React Native](/surfaces/react_native) frontends render their own
...
[generated hooks](/call/from_react) as any other front end, so its
