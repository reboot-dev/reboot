...
In Claude Code, add the Reboot skills marketplace and install the
`reboot-chat-app` skill:
...
The API definition uses the same `Session` + application type
pattern described in the
[hand-written guide](/ai_chat_apps/get_started): `Session` is the
