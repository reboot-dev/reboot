...
and an \`App.tsx\` that reads \`?counter=<id>\` and watches/increments
the counter with the same generated \`useCounter()\` hook.
...
<!-- frontend/web/index.html — standalone browser SPA for the
     ai-chat-counter example's Counter. Pass `?counter=<id>` (or paste
     an ID) to watch and increment that counter. Served from its own
     origin by the Vite dev server, cross-origin to the backend, which
     it reaches via `VITE_REBOOT_URL` (see `src/main.tsx`). -->
<!DOCTYPE html>
<html lang="en">
  <head>
    <meta charset="UTF-8" />
    <meta name="viewport" content="width=device-width, initial-scale=1.0" />
    <title>Counter — ai-chat-counter</title>
  </head>
  <body>
    <div id="root"></div>
...
// production. If it's left unset the provider falls back to
// auto-detecting `window.location.origin`, but for a web app that
// resolves to the app's own origin, the wrong place to reach the API —
// so don't rely on it here.
createRoot(document.getElementById("root")!).render(
  <StrictMode>
...
\`generate --react=frontend/api\` (generates React hooks) and
...
<!-- MARKDOWN-AUTO-DOCS:START (CODE:src=../../../reboot/examples/ai-chat-counter/.rbtrc&lines=1-35&syntax=sh) -->
...
dev run:hmr --frontend-host=http://localhost:4444 --frontend-root-path=frontend
...
dev run:dist --frontend-dist-path=frontend/dist --frontend-root-path=frontend
```
