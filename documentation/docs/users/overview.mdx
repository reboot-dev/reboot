...
1. **Auto-construction.** A `User` instance is created the first time
   ...
   You never call `User.factory().create(...)` yourself.
...
does this through [`useSignIn()`](#on-the-client), an MCP client
...
- `User` methods **list** what this person owns (`list_todo_lists`),
  so a fresh client — a browser after a reload, or an MCP client
...
This pattern pays off on every frontend. A web app renders the list. An
MCP client calls the same method as a tool and remembers the IDs for
...
- **`useSignIn()`**, from `@reboot-dev/reboot-react`, starts the
  ...
  [MCP client](/frontends/mcp_ui) signs the person in when they
...
| [MCP UI](/frontends/mcp_ui) | The MCP client runs the flow itself when the person connects your app. |
