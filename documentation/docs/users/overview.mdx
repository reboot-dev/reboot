...
This pattern pays off on every frontend. A web app renders the list. A
...
- **`useSignIn()`**, from `@reboot-dev/reboot-react`, starts the
  sign-in flow: a redirect in a [browser](/frontends/web#signing-in),
  a system-browser round trip in a
  [React Native app](/frontends/react_native). An
  [AI chat client](/frontends/ai_chat) signs the person in when they
...
| --- | --- |
| [Web](/frontends/web) | `useSignIn()` sends the browser through your identity provider and back. |
| [React Native](/frontends/react_native) | `useSignIn()` opens a system browser for your identity provider; the app passes `nativeAuth({...})` to its provider once, and everything else is the same. |
| [AI chat (MCP)](/frontends/ai_chat) | The chat client runs the flow itself when the person connects your app. |
